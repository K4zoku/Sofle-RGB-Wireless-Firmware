...
#endif
#if DT_NODE_EXISTS(DT_NODELABEL(via_mmv))
#define VIA_MMV_BEHAVIOR DEVICE_DT_NAME(DT_NODELABEL(via_mmv))
#else
...
static bool via_make_binding(uint16_t keycode, struct zmk_behavior_binding *binding) {
    ...
    /* ext_power has no metadata provider in ZMK 0.3, so accept this fixed
     * custom binding when supplied by a consumer keymap. */
    if (strcmp(binding->behavior_dev, VIA_EXT_POWER_BEHAVIOR) == 0 &&
        binding->param1 == EXT_POWER_TOGGLE_CMD && binding->param2 == 0) {
        return true;
    }
